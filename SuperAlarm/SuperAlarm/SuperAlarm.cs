...
namespace SuperAlarm
{
    ...
    public class SuperAlarm
    {
        ...
        public string Time
        {
            get
            {
                return this.StartTime.ToString("hh:mm tt");
            }
        }
    ...
    public class AlarmSchedule
    {
        ...
        {
            string repeats = this.Repeats.ToString();

            if (this.Repeats == RecurrenceInterval.Weekly)
            {
                if (this.DaysToRepeat.Count > 0)
                {
                    if (this.DaysToRepeat.Count == 2 && this.DaysToRepeat.Contains(DayOfWeek.Saturday) && this.DaysToRepeat.Contains(DayOfWeek.Sunday))
                    {
                        repeats = "weekends";
                    }
                    else if (this.DaysToRepeat.Count == 5 && !this.DaysToRepeat.Contains(DayOfWeek.Saturday) && !this.DaysToRepeat.Contains(DayOfWeek.Sunday))
                    {
                        repeats = "weekdays";
                    }
                    else if (this.DaysToRepeat.Count == 7)
                    {
                        repeats = "every day";
                    }
                    else
                    {
                        repeats += " : " + string.Join(", ", this.DaysToRepeat.Select(x => x.ToString().Substring(0, 3)).ToArray());
                    }
                }
            }
            else if (this.Repeats == RecurrenceInterval.None)
            {
                repeats = "Only Once";
            }
